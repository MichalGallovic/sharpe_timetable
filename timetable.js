...
(function() {
  ...
  function prettyFormatHourTwenty(hour) {
    var prefix = hour < 10 ? '0' : '';
    return prefix + hour + ':20';
  }

  function prettyFormatHourForty(hour) {
    var prefix = hour < 10 ? '0' : '';
    return prefix + hour + ':40';
  }
      ...
      function appendColumnHeaders(node) {
        ...
        for (var hour=timetable.scope.hourStart; !completed;) {
          ...
          if (!completed) {
            var spanNode = liNode.appendChild(document.createElement('span'));
            spanNode.className = 'time-label';
            spanNode.textContent = prettyFormatHourTwenty(hour);

            var spanNode = liNode.appendChild(document.createElement('span'));
            spanNode.className = 'time-label';
            spanNode.textContent = prettyFormatHourForty(hour);
          }
